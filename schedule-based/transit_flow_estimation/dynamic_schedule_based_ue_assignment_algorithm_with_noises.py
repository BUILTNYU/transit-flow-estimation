...
def dynamic_schedule_based_ue_assignment_algorithm_with_noises(tn,od_flow, od_flow_to_stop_prob):
	...
	VARIANCE_OF_LOG_PERCENTAGE_NOISE = float(open_config_file('VARIANCE_OF_LOG_PERCENTAGE_NOISE'))
	# The number of hypapths stored.
	num_hyperpaths = int(open_config_file('NUM_HYPERPATH_ENUMERATED'))
	...
	print()
	print("assignment nmodel with noises...")
	print("    T: " + str(T))
	...
	print("    num_hyperpaths: " + str(num_hyperpaths))
	...
	for r in range(num_stops):
		for s in range(num_strategies):
			for hp in range(num_hyperpaths):
				for l in range(num_choices):
					for i_t in range(num_stops_exp):
						...
						for tau in range(temp, t + 1):
							temp_key_set.add((r,s,hp,l,tau,i_t))

	prefer_links = {key: [] for key in temp_key_set}
	prefer_probs = {key: [] for key in temp_key_set}

	# Strategy flow[q,r,h,s]
	# The noise is percentage noise;
	# namely the log(percentage_noise) ~ Normal(0, )
	strategy_flow = np.zeros((num_stops, num_stops, T, num_strategies, num_hyperpaths))

	# Prepare noises
	log_noises = np.random.normal(0, VARIANCE_OF_LOG_PERCENTAGE_NOISE, (num_hyperpaths, num_links_exp))
	noises = np.ones((num_hyperpaths, num_links_exp))
	for temp_1 in range(num_hyperpaths):
		for temp_2 in range(num_links_exp):
			noises[temp_1,temp_2] = math.exp(log_noises[temp_1,temp_2])
		...
		for r in range(num_stops):
			for l in range(num_choices):
				for i_t in range(num_stops_exp):
					...
					for tau in range(temp, t + 1):						
						prefer_links[r,s,hp,l,tau,i_t] = deepcopy(prefer_links_optimal[r,l,tau,i_t])
						prefer_probs[r,s,hp,l,tau,i_t] = deepcopy(prefer_probs_optimal[r,l,tau,i_t])

	# Assign all od_demand to the initial strategy.
	# s still 0.
	# Divide hyperpath flow evenly, although this is not necessary; for the purpose of test.
	for q in range(num_stops):
		for r in range(num_stops):
			for h in range(T):
				for hp in range(num_hyperpaths):
					strategy_flow[q,r,h,s,hp] = od_flow[q,r,h] / num_hyperpaths
	...
	while converg_flag == 0:
		...
		# Initialized for each iteration.
		# [q,r,h,s,hp]
		Y = np.zeros((num_stops, num_stops, T, num_strategies, num_hyperpaths))
				...
				for hp in range(num_hyperpaths):
					for l in range(num_choices):
						for i_t in range(num_stops_exp):
							...
							for tau in range(temp, t + 1):
								for temp_index in range(len(prefer_probs[r,s,hp,l,tau,i_t])):
									prefer_probs[r,s,hp,l,tau,i_t][temp_index] = 0.0
		...
		for i_t in range(num_stops_exp):
			...
			# Node arrival flow, [r,s,hp,l,τ]: no q,h, i_t;
			...
			for s in range(iteration_count_effective):
				for hp in range(num_hyperpaths):
					for r in range(num_stops):
						...
						if strategy_flow[q,r,h,s,hp] > 0.0001:

							node_flow_key = (r,s,hp,l,tau)
							if node_flow_key in node_flow:
								node_flow[node_flow_key] += strategy_flow[q,r,h,s,hp]
							else:
								node_flow[node_flow_key] = strategy_flow[q,r,h,s,hp]
							...
							if TEST_LOAD_STRATEGY_FLOW == 1:
								...
								print("    Collecting flow - Case 1) assign departure flow to nodes - " + "node_flow" +\
								 str(node_flow_key) + ": " + str(strategy_flow[q,r,h,s,hp]))
							# End of test print.
			...
			# link_flow_key[r,s,hp,l,t,outgoing_link]
			for link_flow_key in link_flow:

				incoming_link = link_flow_key[5]
				...
				if ending_node == i_t:
					...
					l_prime = link_flow_key[3]
					tau_prime = link_flow_key[4]
					...
					for hp in range(num_hyperpaths):
						for l in range(num_choices):
							...
							for tau in range(temp, t + 1):
								...
								prefer_links_temp[r,s,hp,l,tau] = deepcopy(prefer_links[r,s,hp,l,tau,i_t])
								prefer_routes_temp[r,s,hp,l,tau] = [tn.links_exp[item][1] for item in prefer_links_temp[r,s,hp,l,tau]]
								prefer_stops_temp[r,s,hp,l,tau] = [tn.links_exp[item][3] for item in prefer_links_temp[r,s,hp,l,tau]]
			...
			for node_flow_key in node_flow:
				...
				if l != (num_choices - 1) and tau == t:
					...
					# Preference set has this route as 1st choice.
					# tau = t.
					if prefer_routes_temp[r,s,hp,l,t] and prefer_routes_temp[r,s,hp,l,t][0] == l:
						...
						prefer_probs[r,s,hp,l,t,i_t][0] = 1.0
						if TEST_LOAD_STRATEGY_FLOW == 1:
							print("    prefer_probs" + str([r,s,hp,l,t,i_t]) + "[0]" + " updated to 1.0;")
						# End of test print.
				...
				while tau_loading_flag == 0:
					...
					for node_flow_key in node_flow:
						...
						if node_flow_key[4] == tau:
							...
							if prefer_links_temp[r,s,hp,l,tau]:										

								temp_index = outgoing_links_index.index(prefer_links_temp[r,s,hp,l,tau][0])
							...
							for node_flow_key in node_flow:
								...
								for tau2 in range(tau,t + 1):
									if l == (num_choices - 1) or (l != (num_choices - 1) and tau2 == t):
										if saturated_link in prefer_links_temp[r,s,hp,l,tau2]:
											temp = prefer_links_temp[r,s,hp,l,tau2].index(saturated_link)
											del prefer_links_temp[r,s,hp,l,tau2][temp]
											del prefer_routes_temp[r,s,hp,l,tau2][temp]
											del prefer_stops_temp[r,s,hp,l,tau2][temp]			
						...
						for node_flow_key in node_flow:
							...
							if node_flow_key[4] == tau:
								...
								if prefer_links_temp[r,s,hp,l,tau]:
									...
									outgoing_link = prefer_links_temp[r,s,hp,l,tau][0]

									temp_flow = node_flow[node_flow_key]

									link_flow_key =  (r,s,hp,l,tau,outgoing_link)
									...
									# Update prob.
									temp = prefer_links[r,s,hp,l,tau,i_t].index(outgoing_link)

									prefer_probs[r,s,hp,l,tau,i_t][temp] = 1.0 - sum(prefer_probs[r,s,hp,l,tau,i_t])
									if TEST_LOAD_STRATEGY_FLOW == 1:
										print()
										print("    prefer_probs" + str([r,s,hp,l,tau,i_t]) + '[' + str(temp) +']' + " updated to " + str(prefer_probs[r,s,hp,l,tau,i_t][temp]))
									# End of test print.
						...
						for node_flow_key in node_flow:
							...
							if node_flow_key[4] == tau:
								if prefer_links_temp[r,s,hp,l,tau]:
									...
									temp_flow = min_ratio * node_flow[r,s,hp,l,tau]
									...
									temp = prefer_links[r,s,hp,l,tau,i_t].index(outgoing_link)
									prefer_probs[r,s,hp,l,tau,i_t][temp] = min_ratio * (1.0 - sum(prefer_probs[r,s,hp,l,tau,i_t]))
							...
							for tau2 in range(tau,t + 1):
								if l == (num_choices - 1) or (l != (num_choices - 1) and tau2 == t):
									if min_ratio_link_index in prefer_links_temp[r,s,hp,l,tau2]:
										temp = prefer_links_temp[r,s,hp,l,tau2].index(min_ratio_link_index)
										del prefer_routes_temp[r,s,hp,l,tau2][temp]
										del prefer_links_temp[r,s,hp,l,tau2][temp]
										del prefer_stops_temp[r,s,hp,l,tau2][temp]			
			...
			starting_nodes_all = {}
			for link_flow_key in link_flow:
				# link_flow[r,s,hp,l,t,outgoing_link]
				outgoing_link_temp = link_flow_key[5]
					...
					for tau in range(t + 1):
						...
						for link_flow_key in link_flow:
							...
							if starting_node2 == i_t:
								...
								l2 = link_flow_key[3]
								tau2 = link_flow_key[4]
								outgoing_link2 = link_flow_key[5]
								outgoing_route2 = outgoing_routes_all[link_flow_key]

								if prefer_links[r2,s2,hp2,l2,tau2,i_t] and outgoing_link in prefer_links[r2,s2,hp2,l2,tau2,i_t]:

									rank_in_prefer_set = prefer_links[r2,s2,hp2,l2,tau2,i_t].index(outgoing_link)
									rank_in_prefer_set2 = prefer_links[r2,s2,hp2,l2,tau2,i_t].index(outgoing_link2)
									...
									if (tau2 == t)\
									 ...
									 and (outgoing_link2 == prefer_links[r2,s2,hp2,l2,tau2,i_t][0]):
									...
									if (tau2 == tau)\
									 and (outgoing_link in prefer_links[r2,s2,hp2,l2,tau2,i_t])\
									 and ((tau != t) or (l2 != tn.links_exp[prefer_links[r2,s2,hp2,l2,tau2,i_t][0]][1]))\
			...
			for hp in range(num_hyperpaths):
				for l in range(num_choices):
					for i_t in range(num_stops_exp):
						...
						for tau in range(temp, t + 1):
							temp_key_set.add((r,hp,l,tau,i_t))

		prefer_links_optimal = {key: [] for key in temp_key_set}
		prefer_probs_optimal = {key: [] for key in temp_key_set}

		# Used to store the prob of users choosing the hyperpaths with noises.
		# [q,r,h,hp]
		hyperpath_prob = np.zeros((num_stops,num_stops,T,num_hyperpaths))
		...
		for r in range(num_stops):
			...
			w_cost = float('inf') * np.ones((num_hyperpaths, num_choices, T, num_stops_exp))
				...
				if i == r:
					...
					# Case 3)
						...
						for tau in range(t + 1):
							...
							phi_cost = {}
							for hp in range(num_hyperpaths):
								phi_cost[hp] = np.zeros((len(outgoing_links),2))

							for hp in range(num_hyperpaths):
								for temp_index in range(len(outgoing_links)):
									phi_cost[hp][temp_index,0] = temp_index
									phi_cost[hp][temp_index,1] = float('inf')
							...
							for temp_index in range(len(outgoing_links)):
								...
								if outgoing_route != (num_choices -1):
									for hp in range(num_hyperpaths):
										phi_cost[hp][temp_index][1] = tn.links_exp[outgoing_link][8] * noises[hp,outgoing_link] + w_cost[hp,outgoing_route,t_cij,downstream_stop]
								else:
									for hp in range(num_hyperpaths):
										phi_cost[hp][temp_index][1] = tn.links_exp[outgoing_link][8] * noises[hp,outgoing_link] + w_cost[hp,outgoing_route,tau,downstream_stop]

							# Sort phi in descending order
							# Note that phi may have infinite value elmts.
							for hp in range(num_hyperpaths):
								phi_cost[hp] = phi_cost[hp][phi_cost[hp][:,1].argsort()]
							...
							if np.amin(phi_cost[0][:,1]) == float('inf'):
							...
							for hp in range(num_hyperpaths): 	
								for temp_index in range(len(outgoing_links)):								
									temp = int(phi_cost[hp][temp_index,0])
									prefer_links_1[hp].append(outgoing_links_index[temp])
									prefer_probs_1[hp].append(0.0)

							# virtual_flow_prob calculation
							# Used to record the assign probability over prefer_links.
							virtual_flow_prob = {}
							for hp in range(num_hyperpaths):
								virtual_flow_prob[hp] = []
							...
							for hp in range(num_hyperpaths):
								...
								for outgoing_link_temp_index in range(len(phi_cost[hp])):

									# If this is an effective arc in preference set ...
									if phi_cost[hp][outgoing_link_temp_index,1] < float('inf'):

										# The index of this downstream node.
										# Note that links are visited according to the seq in phi_cost.
										index_in_outgoing_links = int(phi_cost[hp][outgoing_link_temp_index,0])
										...
										if temp_count == 0:
											virtual_flow_prob[hp].append(prob_of_boarding[i_t,tau,outgoing_route])
										elif temp_count > 0:
											virtual_flow_prob[hp].append((1 - prob_accumulated) * prob_of_boarding[i_t,tau,outgoing_route])
										
										temp_count += 1

										if TEST_FIND_OPTIMAL_STRATEGY == 1:
											print()
											print("    virtual_flow_prob[" + str(hp) +"] updated to:")
											print(virtual_flow_prob[hp])
										# End of test print.

										prob_accumulated = 0
										for temp in range(len(virtual_flow_prob[hp])):
											prob_accumulated += virtual_flow_prob[hp][temp]
							...
							w_cost_1 = np.zeros(num_hyperpaths)
							w_cost_2 = np.zeros(num_hyperpaths)

							for hp in range(num_hyperpaths):
								for temp in range(len(virtual_flow_prob[hp])):
									w_cost_1[hp] += virtual_flow_prob[hp][temp] * phi_cost[hp][temp,1]
							...
							if tau == t:
								...
								if incoming_routes_index:
									for temp in range(len(incoming_routes_index)):
										...
										if (l == (num_choices - 1)) or (l not in outgoing_routes_index):
											for hp in range(num_hyperpaths):
												w_cost[hp,l,tau,i_t] = w_cost_1[hp]
												prefer_links_optimal[r,hp,l,tau,i_t] = deepcopy(prefer_links_1[hp])
												prefer_probs_optimal[r,hp,l,tau,i_t] = deepcopy(prefer_probs_1[hp])
								...
								# In fact for departure flows.
									...
									for hp in range(num_hyperpaths):
										...
										if TEST_FIND_OPTIMAL_STRATEGY == 1:
											print()
											print("    Case 3.1 - prefer_links_optimal" + str([r,hp,l,tau,i_t]) + ":")
											print(prefer_links_optimal[r,hp,l,tau,i_t])
											print("    Case 3.1 - prefer_probs_optimal" + str([r,hp,l,tau,i_t]) + ":")
											print(prefer_probs_optimal[r,hp,l,tau,i_t])
										# End of test print.
								...
								theta = 0.1
								for hp in range(num_hyperpaths):
									exp_sum += math.exp( (- 1) * theta * w_cost_1[hp])
								for hp in range(num_hyperpaths):
									hyperpath_prob[i,r,t,hp] = math.exp((- 1) * theta * w_cost_1[hp]) / exp_sum

								for hp in range(num_hyperpaths):
									C_Y += od_flow[i,r,t] * hyperpath_prob[i,r,t,hp] * w_cost_1[hp]

									if TEST_LOAD_STRATEGY_FLOW == 1 and od_flow[i,r,t] > 0.0001:
										print()
										print("    C_Y got updated from od_flow" + str([i,r,t]) + ' hyperpath_prob[' + str(i,r,t,hp) + '] ' +' * w_cost_1' + ' : ' + \
										 str(od_flow[i,r,t]) + ' * ' + str(hyperpath_prob[i,r,t,hp]) + ' * ' + str(w_cost_1[hp]))
									# End of test print.
							...
							# and it must incude a WT route.
								...
								for temp in range(len(incoming_routes_index)):
									...
									for hp in range(num_hyperpaths):
										...
										if TEST_FIND_OPTIMAL_STRATEGY == 1:
											print()
											print("    Case 3.2 - prefer_links_optimal" + str([r,hp,l,tau,i_t]) + ":")
											print(prefer_links_optimal[r,hp,l,tau,i_t])
											print("    Case 3.2 - prefer_probs_optimal" + str([r,hp,l,tau,i_t]) + ":")
											print(prefer_probs_optimal[r,hp,l,tau,i_t])
										# End of test print.
								...
								for temp_index in range(len(incoming_routes_index)):
									...
									if (l != (num_choices - 1)) and (l in outgoing_routes_index):
										...
										for hp in range(num_hyperpaths):										
											...
											w_cost_2[hp] = outgoing_link_cost * noises[hp,outgoing_link] + w_cost[hp,l,t_cij,downstream_stop]
											...
											# “0.0001” is to avoid numerical precision issues.
											if w_cost_2[hp]  + 0.0001 < w_cost_1[hp]:
												...
												w_cost[hp,l,tau,i_t] = w_cost_2[hp]

												prefer_links_optimal[r,hp,l,tau,i_t] = []
												prefer_probs_optimal[r,hp,l,tau,i_t] = []

												prefer_links_optimal[r,hp,l,tau,i_t].append(outgoing_link)
												prefer_probs_optimal[r,hp,l,tau,i_t].append(0.0)
											...
											if TEST_FIND_OPTIMAL_STRATEGY == 1:
												print("    Case 3.3 - prefer_links_optimal" + str([r,hp,l,tau,i_t]) + ":")
												print(prefer_links_optimal[r,hp,l,tau,i_t])
												print("    Case 3.3 - prefer_probs_optimal" + str([r,hp,l,tau,i_t]) + ":")
												print(prefer_probs_optimal[r,hp,l,tau,i_t])
											# End of test print.
		...
		if converg_flag == 1:
			...
			# link_flow[q,r,h,s,hp,l,τ,link_index]
			...
			for i_t in range(num_stops_exp):					
					...
					for hp in range(num_hyperpaths):
						for r in range(num_stops):
							...
							if strategy_flow[q,r,h,s,hp] > 0.0001:
								...
								node_flow_key = (q,r,h,s,hp,l,tau)
								if node_flow_key in node_flow:
									node_flow[node_flow_key] += strategy_flow[q,r,h,s,hp]
								else:
									node_flow[node_flow_key] = strategy_flow[q,r,h,s,hp]
				...
				for link_flow_key in link_flow:
					...
					if ending_node == i_t:
						...
						l_prime = link_flow_key[5]
						tau_prime = link_flow_key[6]
				...
				for node_flow_key in node_flow:
					...
					if TEST_LOAD_STRATEGY_FLOW == 1:
						...
						print("    prefer_links" + str([r,s,hp,l,tau,i_t]) + ":")
						print(prefer_links[r,s,hp,l,tau,i_t])
						print("    prefer_probs" + str([r,s,hp,l,tau,i_t]) + ":")
						print(prefer_probs[r,s,hp,l,tau,i_t])
					# End of test print.
		
					# Assign according to the preference set.
					for temp_index in range(len(prefer_links[r,s,hp,l,tau,i_t])):

						# If this link is used...
						if prefer_probs[r,s,hp,l,tau,i_t][temp_index] >= 0.0001:
							outgoing_link = prefer_links[r,s,hp,l,tau,i_t][temp_index]

							link_flow_key = (q,r,h,s,hp,l,tau,outgoing_link)
							if link_flow_key in link_flow:
								link_flow[link_flow_key] += node_flow[node_flow_key] * prefer_probs[r,s,hp,l,tau,i_t][temp_index]
							else:
								link_flow[link_flow_key] = node_flow[node_flow_key] * prefer_probs[r,s,hp,l,tau,i_t][temp_index]
		...
		if converg_flag == 0:
						...
						for l in range(num_choices):
							...
							for i_t in range(num_stops_exp):
								...
								for tau in range(temp, t + 1):
									...
									if prefer_links_optimal[r,hp,l,tau,i_t] != prefer_links[r,s,hp,l,tau,i_t]:
										coincidence_flag = 0
			...
			for q in range(num_stops):
				for r in range(num_stops):
					for h in range(T):
						for hp in range(num_hyperpaths):
							Y[q,r,h,strategy_index_of_Y,hp] = hyperpath_prob[q,r,h,hp] * od_flow[q,r,h]
					...
					for hp in range(num_hyperpaths):
						for l in range(num_choices):
							for i_t in range(num_stops_exp):
								...
								for tau in range(temp, t + 1):
									prefer_links[r,strategy_index_of_Y,hp,l,tau,i_t] = deepcopy(prefer_links_optimal[r,hp,l,tau,i_t])
									prefer_probs[r,strategy_index_of_Y,hp,l,tau,i_t] = deepcopy(prefer_probs_optimal[r,hp,l,tau,i_t])
